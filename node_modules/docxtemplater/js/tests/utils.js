...
var safeStringify = require("../safe-json-stringify.js");
...
function zipCompare(zip, expectedZip, expectedName) {
  ...
  for (var _i6 = 0; _i6 < filePaths.length; _i6++) {
    var filePath = filePaths[_i6];
...
function checkLength(e, expectedError, propertyPath) {
  ...
  if (property && expectedPropertyLength) {
    expect(expectedPropertyLength).to.be.a("number", safeStringify(expectedError.properties));
...
function cleanRecursive(arr) {
  ...
  for (var _i8 = 0; _i8 < arr.length; _i8++) {
    ...
    if (p.value instanceof Array) {
      for (var _i0 = 0, _p$value2 = p.value; _i0 < _p$value2.length; _i0++) {
        var part = _p$value2[_i0];
        cleanRecursive(part);
      }
    }
...
function cleanError(e, expectedError) {
  ...
  if (expectedError.properties && e.properties) {
    ...
    if (expectedError.properties.explanation != null) {
      ...
      expect(e1).to.deep.equal(e2, "Explanations differ '".concat(e1, "' != '").concat(e2, "': for ").concat(safeStringify(expectedError)));
    }
    ...
    if (e.properties.rootError) {
      expect(e.properties.rootError, safeStringify(e.properties)).to.be.instanceOf(Error, "rootError doesn't have correct type");
      expect(expectedError.properties.rootError, safeStringify(expectedError.properties)).to.be.instanceOf(Object, "expectedError doesn't have a rootError");
      if (expectedError) {
    ...
    if (expectedError.properties.offset != null) {
      ...
      expect(o1).to.be.deep.equal(o2, "Offset differ ".concat(o1, " != ").concat(o2, ": for ").concat(safeStringify(expectedError)));
    }
...
function jsonifyError(e) {
  return JSON.parse(safeStringify(e, function (key, value) {
...
function errorVerifier(e, type, expectedError) {
  ...
  if (e.properties.errors) {
    ...
    for (var i = 0; i < e.properties.errors.length; i++) {
      var suberror = e.properties.errors[i];
    ...
    if (writeSnapshots) {
      ...
      for (var i = 0; i < sortedKeys.length; i++) {
        ...
        if (typeof snap === "string") {
          ...
          parts.push("exports[`" + key + "`] = " + JSON.stringify(snap, null, 2));
        }
      }
      output += parts.join("\n\n") + "\n\n";
...
function start() {
  ...
  var _loop = function _loop() {
    ...
    if (endsWithOne(fileName, [".docm", ".docx", ".odt", ".ods", ".odp", ".dotm", ".dotx", ".potm", ".potx", ".pptm", ".pptx", ".xlsm", ".xlsx", ".xltm", ".xltx"])) {
...
function setExamplesDirectory(ed) {
  ...
  if (fs && fs.writeFileSync) {
    var fileNames = [];
    var walkResults = walk(examplesDirectory);
    for (var _i16 = 0; _i16 < walkResults.length; _i16++) {
      var f = walkResults[_i16];
...
function createDocV3(name) {
  ...
  if (options.modules) {
    for (var _i18 = 0, _options$modules2 = options.modules; _i18 < _options$modules2.length; _i18++) {
      var _module = _options$modules2[_i18];
...
function createDocV4(name) {
  ...
  var moduleNames = [];
  for (var _i20 = 0, _options$modules4 = options.modules; _i20 < _options$modules4.length; _i20++) {
    var _module2 = _options$modules4[_i20];
    moduleNames.push(_module2.name);
  }
...
function paragraph(content) {
  ...
  return "<w:p><w:r><w:t xml:space=\"preserve\">".concat(content, "</w:t></w:r></w:p>");
}
function cell(content) {
  return "<w:tc>".concat(paragraph(content), "</w:tc>");
}
function runWithoutProxy(fn) {
  var previousProxy = global.Proxy;
