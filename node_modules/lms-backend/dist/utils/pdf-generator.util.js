 ...
 */
const generateDiagnosticReportPDF = async (records) => {
    return new Promise((resolve, reject) => {
        ...
        const sheets = (Array.isArray(records) ? records : [records]).filter(Boolean);
        ...
        sheets.forEach((sheet, index) => {
            ...
            if (index > 0 && doc.y > 620)
                doc.addPage();
            if (index > 0)
                doc.moveDown(1);
            doc.fillColor('#111827').font('Helvetica-Bold').fontSize(11).text(`${test.testName || 'Diagnostic Test'}${test.testCode ? ` (${test.testCode})` : ''}`, LEFT, doc.y);
            const meta = [
            ...
            if (rows.length === 0) {
                doc.font('Helvetica-Oblique').fontSize(9).fillColor('#6b7280').text('No parameters are configured for this test. Add them on the test master and re-open the report.', LEFT, doc.y, { width: RIGHT - LEFT });
            }
            rows.forEach((res) => {
                ...
                const marker = (0, result_marker_util_1.resultMarker)(res);
            ...
            if (rows.some((res) => (res.flag || 'Normal') !== 'Normal')) {
                doc.moveDown(0.4);
                doc
                    .font('Helvetica')
                    .fontSize(7)
        ...
        doc.moveDown(0.6);
        const signTop = doc.y;
        doc.font('Helvetica-Bold').fontSize(9).fillColor('#111827').text(resultRecord.enteredBy?.name || 'Technician', LEFT, signTop, { width: 250 });
        doc.font('Helvetica').fontSize(8).fillColor('#6b7280').text(resultRecord.enteredBy?.role || 'Lab Technician', LEFT, doc.y, { width: 250 });
        ...
        const verifiers = Array.from(new Set(sheets.map((sheet) => sheet.verifiedBy?.name).filter(Boolean)));
        const verifiedAt = sheets.find((sheet) => sheet.verifiedBy?.date)?.verifiedBy?.date;
        doc.font('Helvetica-Bold').fontSize(9).fillColor('#111827').text(verifiers.length ? verifiers.join(', ') : 'Pending verification', 320, signTop, { width: 250, align: 'right' });
        doc.font('Helvetica').fontSize(8).fillColor('#6b7280').text(verifiers.length > 1 ? 'Consultant Pathologists' : 'Consultant Pathologist', 320, doc.y, { width: 250, align: 'right' });
        ...
        doc.fontSize(7).text('Results relate only to the samples tested. Not valid for medico-legal purposes.', LEFT, doc.y, { width: RIGHT - LEFT, align: 'center' });
        doc.end();
    });
};
exports.generateDiagnosticReportPDF = generateDiagnosticReportPDF;
exports.generateResultPDF = exports.generateDiagnosticReportPDF;
