...
interface CatalogueEntry {
  /** Any of these appearing in the test name or code claims the entry. */
  keywords: string[];
  /**
   * Higher wins a tie against a longer but vaguer keyword. Single-analyte
   * entries whose name is a substring of bigger panels sit at 1 so that
   * "Glycated Haemoglobin" lands on HbA1c rather than on plain haemoglobin.
   */
  priority?: number;
  parameters: Omit<ParameterTemplate, 'displayOrder'>[];
}
...
const num = (
  ...
  male: string,
  female = male,
  extra: Partial<ParameterTemplate> = {}
): Omit<ParameterTemplate, 'displayOrder'> => ({
  parameterName,
  shortName,
  unit,
 ...
 */
const genericSheet = (testName: string): ParameterTemplate[] => [
  {
 ...
 */
export const findParameterTemplate = (testName: string, testCode = ''): ParameterTemplate[] => {
