...
const DEPARTMENTS: Record<CheckupDept, { departmentName: string; description: string }> = {
...
const run = async () => {
  ...
  const deptIds = new Map<CheckupDept, any>();
  const needed = [...new Set(CHECKUP_PANELS.map((p) => p.dept))];
  ...
  for (const panel of CHECKUP_PANELS) {
    const already =
      byCode.get(panel.testCode.toUpperCase()) || byName.get(panel.testName.trim().toLowerCase());
    ...
    if (already) {
      ...
      console.log(
        `= ${panel.testName} - already on the master as ${already.testCode}, left as it is` +
    ...
    const doc = {
      ...
      processingMode: 'In-house' as const,
      outsourceLab: '',
      outsourceCost: 0,
    ...
    if (dryRun) {
      console.log(
        `[dry-run] + ${panel.testName} (${panel.testCode}) - ${rupees(panel.rate)}, ${panel.parameters.length} parameter(s)`
      );
      created += 1;
      wouldInclude += 1;
      continue;
    ...
    created += 1;
    console.log(
      `+ ${panel.testName} (${panel.testCode}) - ${rupees(panel.rate)}, ${panel.parameters.length} parameter(s)`
    );
  }
  ...
  const panelTotal = CHECKUP_LIST_TOTAL;
  const saving = panelTotal - CHECKUP_PACKAGE.rate;

  console.log('\n--- Summary ---');
  console.log(`Panels in this file     : ${CHECKUP_PANELS.length}`);
  console.log(`${dryRun ? 'Would add' : 'Added'}               : ${created}`);
  console.log(`Already on the master   : ${reused}`);
  if (updateSheets) console.log(`Sheets replaced         : ${resheeted}`);
  if (unplaced) console.log(`Skipped (no department) : ${unplaced}`);
  console.log(`Parameters in the set   : ${CHECKUP_PANELS.reduce((s, p) => s + p.parameters.length, 0)}`);
  console.log(`Panels billed one by one: ${rupees(panelTotal)}`);
  console.log(
    `Package price           : ${rupees(CHECKUP_PACKAGE.rate)}` +
