...
interface MenuEntry {
  ...
  dept: Dept;
  rate: number;
  /** Serum unless the test is run on whole blood, urine or a swab. */
  sampleType?: string;
  sampleContainer?: string;
  fasting?: boolean;
  tat?: string;
  testType?: 'Routine' | 'Special' | 'Urgent' | 'Profile';
}

const DEPARTMENTS: Record<Dept, { departmentName: string; description: string }> = {
...
const run = async () => {
  ...
  for (const entry of MENU) {
    ...
    toCreate.push({
      ...
      parameters: parametersForTest(entry.testName, entry.testCode),
    });
  }
  ...
  toCreate.forEach((t) =>
    console.log(
      `${dryRun ? '[dry-run] ' : ''}+ ${t.testName} (${t.testCode}) - Rs.${t.rate}, ${t.parameters.length} parameter(s)`
    )
  );
