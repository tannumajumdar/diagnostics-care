...
const run = async () => {
  ...
  if (asCsv) {
    ...
    patients.forEach((p: any) => {
      console.log(
        [p.uhid, p.patientName, p.gender, String(p.age), p.mobile, p.status, asDate(p.registrationDate)]
          .map((v) => csvCell(v ?? '-'))
          .join(',')
      );
    });
    ...
    console.log(
      `${pad('UHID', 20)}${pad('PATIENT NAME', 28)}${pad('GENDER', 14)}${pad('AGE', 5)}${pad('MOBILE', 14)}${pad('STATUS', 12)}REGISTERED`
    );
    console.log('-'.repeat(105));
    patients.forEach((p: any) => {
      console.log(
        `${pad(p.uhid ?? '-', 20)}${pad(p.patientName ?? '-', 28)}${pad(p.gender ?? '-', 14)}${pad(String(p.age ?? '-'), 5)}${pad(p.mobile ?? '-', 14)}${pad(p.status ?? '-', 12)}${asDate(p.registrationDate)}`
      );
    });
    ...
    const missing = patients.filter((p: any) => !p.uhid);
    if (missing.length) {
      console.log(`\n${missing.length} patient(s) have NO UHID:`);
      missing.forEach((p: any) => console.log(`  - ${p.patientName} (${p._id})`));
    }

    const seen = new Map<string, number>();
    patients.forEach((p: any) => p.uhid && seen.set(p.uhid, (seen.get(p.uhid) ?? 0) + 1));
