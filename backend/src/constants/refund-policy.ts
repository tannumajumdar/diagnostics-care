...
export type RefundStage = (typeof REFUND_STAGES)[keyof typeof REFUND_STAGES];

export const REFUND_STAGE_ORDER: RefundStage[] = [
  REFUND_STAGES.BEFORE_COLLECTION,
  REFUND_STAGES.AFTER_COLLECTION,
  REFUND_STAGES.AFTER_PROCESSING,
  REFUND_STAGES.AFTER_REPORT,
];

/** What each stage means at the bench, in the words the counter would use. */
export const REFUND_STAGE_LABELS: Record<RefundStage, { label: string; description: string }> = {
  [REFUND_STAGES.BEFORE_COLLECTION]: {
    label: 'Before the sample is drawn',
    description:
      'Billed, but nothing has been taken from the patient yet. The centre has spent nothing on this test.',
  },
  [REFUND_STAGES.AFTER_COLLECTION]: {
    label: 'Sample drawn, not yet on the bench',
    description:
      'The vial has been drawn and accessioned. Consumables are gone; the analyser has not been run.',
  },
  [REFUND_STAGES.AFTER_PROCESSING]: {
...
export const DEFAULT_REFUND_POLICY = {
  enabled: true,
  stages: {
    [REFUND_STAGES.BEFORE_COLLECTION]: { allowed: true, refundPercent: 100 },
    [REFUND_STAGES.AFTER_COLLECTION]: { allowed: true, refundPercent: 50 },
    [REFUND_STAGES.AFTER_PROCESSING]: { allowed: true, refundPercent: 25 },
    [REFUND_STAGES.AFTER_REPORT]: { allowed: false, refundPercent: 0 },
  },
  cancellationFee: 0,
  refundWindowDays: 30,
  /**
  ...
  policyNote:
    'Tests cancelled before the sample is drawn are refunded in full. Once a sample has been ' +
