...
class TestController {
    static getAll = async (req, res, next) => {
        try {
            ...
            const [tests, total] = await Promise.all([
                test_model_1.LabTest.find(filter).populate('department').sort({ testName: 1 }).skip(skip).limit(Number(limit)),
                test_model_1.LabTest.countDocuments(filter),
            ]);
    ...
    static getById = async (req, res, next) => {
        try {
            ...
            const test = await test_model_1.LabTest.findById(id).populate('department');
            if (!test)
                throw new api_error_util_1.ApiError(messages_1.HTTP_STATUS.NOT_FOUND, 'Test not found');
            (0, api_response_util_1.sendResponse)({ res, statusCode: messages_1.HTTP_STATUS.OK, message: 'Test details retrieved', data: test });
        }
        catch (error) {
            next(error);
        }
    ...
    static create = async (req, res, next) => {
        try {
            ...
            if (!Array.isArray(body.parameters) || body.parameters.length === 0) {
                body.parameters = (0, test_parameters_1.parametersForTest)(body.testName, body.testCode);
            }
            ...
            if (!String(body.sampleType || '').trim())
                body.sampleType = 'Whole Blood';
            if (!String(body.sampleContainer || '').trim())
                body.sampleContainer = 'EDTA Vial';
            if (!String(body.turnaroundTime || '').trim())
                body.turnaroundTime = '24 Hours';
            const test = await test_model_1.LabTest.create(body);
            (0, api_response_util_1.sendResponse)({ res, statusCode: messages_1.HTTP_STATUS.CREATED, message: 'Lab test created', data: test });
        }
        catch (error) {
            next(error);
        }
    ...
    static update = async (req, res, next) => {
        try {
            ...
            const test = await test_model_1.LabTest.findByIdAndUpdate(id, body, { new: true });
            if (!test)
                throw new api_error_util_1.ApiError(messages_1.HTTP_STATUS.NOT_FOUND, 'Test not found');
            (0, api_response_util_1.sendResponse)({ res, statusCode: messages_1.HTTP_STATUS.OK, message: 'Lab test updated', data: test });
        }
        catch (error) {
            next(error);
        }
    };
    static updateRates = async (req, res, next) => {
        try {
            const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            const currentUser = req.user;
            const test = await test_model_1.LabTest.findById(id);
            if (!test)
                throw new api_error_util_1.ApiError(messages_1.HTTP_STATUS.NOT_FOUND, 'Test not found');
            const previousRates = {
                rate: test.rate,
            ...
            await test.save();
            await rateHistory_model_1.RateHistory.create({
                test: test._id,
            ...
            (0, api_response_util_1.sendResponse)({ res, statusCode: messages_1.HTTP_STATUS.OK, message: 'Rates updated', data: test });
        }
        catch (error) {
            next(error);
        }
    };
    static updateParameters = async (req, res, next) => {
        try {
            const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            const test = await test_model_1.LabTest.findById(id);
            if (!test)
            ...
            await test.save();
            (0, api_response_util_1.sendResponse)({ res, statusCode: messages_1.HTTP_STATUS.OK, message: 'Test parameters updated', data: test });
        }
        catch (error) {
            next(error);
        }
    ...
    static remove = async (req, res, next) => {
        try {
            ...
            const [invoices, samples, results, appointments] = await Promise.all([
                invoice_model_1.Invoice.countDocuments({ 'items.test': test._id }),
                sample_model_1.Sample.countDocuments({ test: test._id }),
                result_model_1.Result.countDocuments({ test: test._id }),
                appointment_model_1.Appointment.countDocuments({ tests: test._id }),
            ]);
            const used = [
            ...
            if (used.length) {
                throw new api_error_util_1.ApiError(messages_1.HTTP_STATUS.CONFLICT, `${test.testName} is already on ${used.join(', ')} and cannot be deleted. ` +
            ...
            await rateHistory_model_1.RateHistory.deleteMany({ test: test._id });
    ...
    static toggleStatus = async (req, res, next) => {
        try {
            ...
            await test.save();
            (0, api_response_util_1.sendResponse)({ res, statusCode: messages_1.HTTP_STATUS.OK, message: 'Status updated', data: test });
        }
        catch (error) {
            next(error);
        }
