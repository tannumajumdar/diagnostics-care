...
export const NewVisitPage: React.FC = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 rounded-xl sm:rounded-2xl border border-slate-200 bg-white p-2.5 sm:p-3 shadow-xs">
        <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto scrollbar-none pb-1 sm:pb-0 min-w-0 flex-1">
          <span className="hidden sm:inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 mr-1 pl-1 shrink-0">
          ...
          {drafts.map((draft, idx) => {
            ...
            return (
              <div
                key={draft.id}
                className={`group shrink-0 flex items-center gap-1.5 sm:gap-2 rounded-xl border px-2.5 sm:px-3 py-1.5 text-xs font-semibold transition-all ${
                  isActive
              ...
              >
                ...
                >
                  ...
                  <span className="max-w-[120px] sm:max-w-[140px] truncate">{tabTitle}</span>
                  {testCount > 0 && (
                    <span
          ...
          <button
            type="button"
            onClick={addNewDraft}
            className="shrink-0 inline-flex items-center gap-1.5 rounded-xl border border-dashed border-indigo-300 bg-indigo-50/60 px-2.5 sm:px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:border-indigo-500 hover:bg-indigo-100 transition-all"
        ...
        <div className="flex items-center gap-2 shrink-0 border-t border-slate-100 sm:border-t-0 pt-2 sm:pt-0">
          <button
            type="button"
            onClick={addNewDraft}
            className="inline-flex w-full sm:w-auto justify-center items-center gap-1.5 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 sm:py-1.5 text-xs font-semibold text-amber-800 hover:bg-amber-100 hover:border-amber-400 transition-colors shadow-2xs"
